...
export function Navigation() {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 flex justify-center transition-all duration-300 pointer-events-none ${isFloating ? 'pt-4 md:pt-6' : 'pt-0'
        }`}
    >
      <div
        className={`pointer-events-auto w-full transition-all duration-300 flex items-center justify-between ${isFloating
      ...
      >
        ...
        <div className="flex items-center">
          <Link href="/" className="flex items-center gap-3 group">
            <Image
              ...
              priority
            />
            <span className={`font-sans font-medium tracking-tight text-foreground transition-all duration-300 ${isFloating ? 'text-small' : 'text-lg'}`}>
              Orizenn
            </span>
          </Link>
        </div>
        ...
        <div className="flex items-center">
          <a
            ...
            className={`font-mono font-medium text-foreground border border-border rounded transition-all hover:bg-surface hover:border-white/20 ${isFloating ? 'text-[11px] px-3 py-1.5' : 'text-micro px-4 py-2'
              }`}
          >
            Open App ↗
          </a>
